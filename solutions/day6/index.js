...
const CheckNum = ({nums})=>{
  const listnum = nums.map(num=>{
      if(isPrime(num)){
        return <span style={{background:'#fd5e53'}} className='number-box'>{num}</span>
      }else if (num%2===0){
        return <span style={{background:'#21bf73'}} className='number-box'>{num}</span>
      }else {
        return <span style={{background:'#fddb3a'}} className='number-box'>{num}</span>
      }
  })
  return (listnum)
}
const NumberBlock = ({nums})=>{
  return (
    <div>
      <h1 className='title'>Number Generator</h1>
      <div className='box-wrapper'><CheckNum nums = {nums}/></div>
    </div>
  )
}
...
function generalColorBlock(numberOfBlock){
  numberOfBlock++
  const colors=[]
...
const ColorBlock = ({colors})=>{
  const colorBlock = colors.map(color=>{
    return (
      <span className='color-box' style={{background:color}}>{color}</span>
    )
  })
  return(
    <div>
...
const Render = ({countries:{country,population}})=>{
  return (
    <div className='population-item'>
      ...
      <div style={{width:800}} className='column'><div style={{width:(population/7693165599)*800}} className='inner'></div></div>
      <span className='population'>{population.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</span>
    </div>
  )
}
const PopulationBlock = ({countryData})=>{
  const listCountry = countryData.map(countries=>{
    return (
      <Render countries={countries}/>
    )
  })
  return (<div className='population-block'>
